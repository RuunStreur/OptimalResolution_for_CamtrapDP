...
# simulation_results <- simulate_multiple_deployments(num_deployments=4, deployment_duration = 365, num_observations = 20000, target_window = 60 , sdlog = 2, num_species = 30)
  

#7 days
# simulate_multiple_deployments <- function(num_deployments, deployment_duration, num_observations, target_window, sdlog, num_species = 30) {
#   all_observations <- tibble()
#   deployments_data <- tibble()
# 
#   rarity_scores_log_normal <- rlnorm(num_species, meanlog = 0, sdlog = sdlog)
...
#     base_rarity_scores <- rarity_scores_log_normal  # Save original scores for resetting
# 
#     for (i in seq_along(dates)) {
...
#         species_spotted[[window_index]] <- character()  # Reset for the new window
#       }
# 
#       daily_obs_count <- sample(round(avg_daily_observations * 0.3):round(avg_daily_observations * 1.7), 1)
#       observed_species <- sample(species_names, size = daily_obs_count, replace = TRUE, prob = rarity_scores_log_normal)
...
whole_pipeline <- function(n_runs, num_species, num_deployments, num_observations_per_deployment, sdlog, target_window_size, min_window, max_window, step_size, species_asymptote_threshold, reached_asymptote_ratio_threshold) {
...
optimal_results <- whole_pipeline(n_runs = 1, 
                                  num_deployments = 4, 
                                  num_species = 30, 
